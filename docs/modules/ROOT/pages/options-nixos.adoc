...
The backend must configure a ``[backend.shared]`` section with a ``serialize`` script to handle serialization for all targets at once{zwsp}. The script receives ``$machines`` and ``$users`` environment variables pointing to JSON files that map target names to their respective ``artifacts{zwsp}.config{zwsp}.{zwsp}+<+{zwsp}backend{gt}`` configurations{zwsp}.
