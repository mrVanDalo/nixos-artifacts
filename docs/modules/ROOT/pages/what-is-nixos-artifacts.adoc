...
Shared artifacts allow you to generate a secret once and distribute it to multiple machines and/or home-manager configurations. This is useful for secrets that must be identical across systems, such as:

- Cluster authentication tokens
- Shared encryption keys
- OIDC client secrets used by multiple services

To create a shared artifact, set `shared = true` in the artifact definition on each target that should receive it:
