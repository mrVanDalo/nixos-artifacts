...
Use the tool to drive the main loop for generation and rotation. See xref:configure-nixos-artifacts.adoc[Configure nixos-artifacts] for configuration and usage.
...
NOTE: The backend must support shared serialization via a `[backend.shared]` section with `serialize` script. See xref:backend-scripts-reference.adoc#_shared_serialize[Scripts Reference] for backend requirements.
