...
- xref:options-nixos.adoc#_artifacts_store[artifacts.store] — What are the artifacts and where should they end up on the target system?
- `artifacts.config` — How do we customize or parameterize backend behavior and define defaults? This is backend-specific.
(See xref:latest@nixos-artifacts-agenix:ROOT:options-nixos.adoc[agenix backend] for example).
...
NOTE: Have a look at xref:backend-quickstart.adoc[Backend Quickstart], if you want to write your own backend.

== TUI and workflow

Use the TUI to drive the main loop for generation and rotation. See xref:configure-nixos-artifacts.adoc[Configure nixos-artifacts] for configuration and usage.

== Shared artifacts

Shared artifacts allow you to generate a secret once and distribute it to multiple NixOS machines. This is useful for secrets that must be identical across systems, such as:

- Cluster authentication tokens
- Shared encryption keys
- OIDC client secrets used by multiple services

NOTE: Shared artifacts are a NixOS-only feature. The Home Manager module does not expose the `shared` option — HM artifacts are per-user and are never aggregated across multiple targets.

To create a shared artifact, set `shared = true` in the artifact definition on each NixOS machine that should receive it:

[source,nix]
----
# In nixosConfigurations.server-one
artifacts.store.cluster-token = {
  shared = true;
  files.token = { };
  generator = ./gen-token.sh;
};

# In nixosConfigurations.server-two
artifacts.store.cluster-token = {
  shared = true;
  files.token = { };
  generator = ./gen-token.sh;
...
All definitions with the same name and `shared = true` are aggregated. The TUI displays shared artifacts as a single entry showing all targets. See xref:artifact-definition-example.adoc#shared-artifacts[Shared artifact example] for more details.

NOTE: The backend must support shared serialization via a `[backend.shared]` section with `check` and `serialize` scripts. See xref:backend-scripts-reference.adoc#_serialize_scripts[Scripts Reference] for backend requirements.
