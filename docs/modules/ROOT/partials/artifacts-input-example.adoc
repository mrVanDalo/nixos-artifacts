  ...
  outputs = inputs@{ self, ... }: {
    nixosConfigurations.my-host = nixpkgs.lib.nixosSystem {
      ...
      modules = [
        ...
        ({ config, pkgs, ... }: {
          artifacts.default.backend = "agenix"; <5>
...
<5> use agenix as the default backend. Every artifact whose xref:options-nixos.adoc#_artifacts_store_name_backend[backend] field is not set explicitly falls back to this default; individual artifacts may override it to use a different backend.
<6> xref:latest@nixos-artifacts-agenix:ROOT:options-nixos.adoc[nixos-artifacts-agenix configuration options]
