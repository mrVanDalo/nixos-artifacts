...
<5> use agenix as the default backend. Every artifact whose xref:options-nixos.adoc#_artifacts_store_name_serialization[serialization] field is not set explicitly falls back to this default; individual artifacts may override it to use a different backend.
