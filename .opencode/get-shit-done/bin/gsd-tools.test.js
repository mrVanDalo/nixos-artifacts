...
`,
    ...
    assert.deepStrictEqual(output.plans[0].files_modified, [
...
`,
    ...
    assert.deepStrictEqual(output.key_files, [
      "prisma/schema.prisma",
      "src/lib/db.ts",
    ], "key files extracted");
    assert.deepStrictEqual(
    ...
    assert.deepStrictEqual(output.patterns, [
      "Repository pattern",
      "Dependency injection",
    ], "patterns extracted");
...
describe("init commands with --include flag", () => {
  ...
  test("init execute-phase includes state and config content", () => {
    ...
    const result = runGsdTools(
      "init execute-phase 03 --include state,config",
      tmpDir,
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(
      output.state_content.includes("Current Phase"),
      "state content correct",
    );
    assert.ok(output.config_content, "config_content should be included");
    assert.ok(
      output.config_content.includes("model_profile"),
  ...
  test("init execute-phase without --include omits content", () => {
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(
      output.state_content,
      undefined,
      "state_content should be omitted",
    );
    assert.strictEqual(
      output.config_content,
      undefined,
      "config_content should be omitted",
    );
  });
  ...
  test("init plan-phase includes multiple file contents", () => {
    ...
    assert.ok(
      output.state_content.includes("Project State"),
      "state content correct",
    );
    assert.ok(output.roadmap_content, "roadmap_content included");
    assert.ok(
      output.roadmap_content.includes("Roadmap v1.0"),
      "roadmap content correct",
    );
  ...
  test("init plan-phase includes verification and uat content", () => {
    ...
    const result = runGsdTools(
      "init plan-phase 03 --include verification,uat",
      tmpDir,
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(
      output.uat_content.includes("UAT Findings"),
      "uat content correct",
    );
  });
...
describe("milestone complete command", () => {
  ...
  test("archives roadmap, requirements, creates MILESTONES.md", () => {
    ...
    const result = runGsdTools(
      "milestone complete v1.0 --name MVP Foundation",
      tmpDir,
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe("scaffold command", () => {
  ...
  test("scaffolds phase directory", () => {
    const result = runGsdTools(
      "scaffold phase-dir --phase 5 --name User Dashboard",
      tmpDir,
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
