  ...
  for (const line of lines) {
    ...
    if (keyMatch) {
      ...
      if (value === "" || value === "[") {
        ...
        current.obj[key] = value.slice(1, -1).split(",").map((s) =>
          s.trim().replace(/^["']|["']$/g, "")
        ).filter(Boolean);
        current.key = null;
      } else {
      ...
      if (
        typeof current.obj === "object" && !Array.isArray(current.obj) &&
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (Array.isArray(value)) {
      if (value.length === 0) {
        ...
        value.every((v) => typeof v === "string") && value.length <= 3 &&
      ...
      for (const [subkey, subval] of Object.entries(value)) {
        ...
        if (Array.isArray(subval)) {
          if (subval.length === 0) {
            ...
            subval.every((v) => typeof v === "string") && subval.length <= 3 &&
      ...
      if (
        sv.includes(":") || sv.includes("#") || sv.startsWith("[") ||
        sv.startsWith("{")
      ) {
        lines.push(`${key}: "${sv}"`);
      } else {
...
function cmdHistoryDigest(cwd, raw) {
  ...
  try {
    ...
    for (const dir of phaseDirs) {
      ...
      const summaries = fs.readdirSync(dirPath).filter((f) =>
...
function cmdPhaseNextDecimal(cwd, basePhase, raw) {
  ...
  try {
    ...
    const baseExists = dirs.some((d) =>
      d.startsWith(normalized + "-") || d === normalized
    );
...
function cmdStateAdvancePlan(cwd, raw) {
  ...
  if (currentPlan >= totalPlans) {
    ...
    content = stateReplaceField(content, "Current Plan", String(newPlan)) ||
      content;
    content = stateReplaceField(content, "Status", "Ready to execute") ||
      content;
    content = stateReplaceField(content, "Last Activity", today) || content;
    fs.writeFileSync(statePath, content, "utf-8");
    output(
...
function cmdStateRecordMetric(cwd, options, raw) {
  ...
  if (metricsMatch) {
    ...
    const newRow = `| Phase ${phase} P${plan} | ${duration} | ${
      tasks || "-"
    } tasks | ${files || "-"} files |`;
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  ...
  const phaseFiles = fs.readdirSync(phaseDir);
  const planFiles = phaseFiles.filter((f) =>
    f.endsWith("-PLAN.md") || f === "PLAN.md"
  ).sort();
  const summaryFiles = phaseFiles.filter((f) =>
    f.endsWith("-SUMMARY.md") || f === "SUMMARY.md"
  );
...
function cmdFrontmatterValidate(cwd, filePath, schemaName, raw) {
  ...
  if (!schema) {
    error(
      `Unknown schema: ${schemaName}. Available: ${
        Object.keys(FRONTMATTER_SCHEMAS).join(", ")
      }`,
    );
  }
  const fullPath = path.isAbsolute(filePath)
...
function cmdVerifyReferences(cwd, filePath, raw) {
  ...
  for (const ref of backtickRefs) {
    ...
    if (
      cleanRef.startsWith("http") || cleanRef.includes("${") ||
      cleanRef.includes("{{")
    ) continue;
      ...
      try {
        ...
        if (regex.test(sourceContent)) {
          ...
          if (targetContent && regex.test(targetContent)) {
            ...
            check.detail =
              `Pattern "${link.pattern}" not found in source or target`;
          }
        }
      } catch {
...
function cmdRoadmapAnalyze(cwd, raw) {
  ...
  while ((match = phasePattern.exec(content)) !== null) {
    ...
    try {
      ...
      if (dirMatch) {
        ...
        hasContext = phaseFiles.some((f) =>
          f.endsWith("-CONTEXT.md") || f === "CONTEXT.md"
        );
        hasResearch = phaseFiles.some((f) =>
  ...
  const completedPhases =
    phases.filter((p) => p.disk_status === "complete").length;
...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  if (lastSeparator > 0) {
    updatedContent = content.slice(0, lastSeparator) + phaseEntry +
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  const nextDecimal = existingDecimals.length === 0
    ? 1
  ...
  const updatedContent = content.slice(0, insertIdx) + phaseEntry +
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  if (fs.existsSync(statePath)) {
    ...
    stateContent = stateContent.replace(
      ...
      `$1Phase ${phaseNum} complete${
        nextPhaseNum ? `, transitioned to Phase ${nextPhaseNum}` : ""
      }`,
    );
    ...
    for (const dir of dirs) {
      ...
      for (let i = 1; i < planNums.length; i++) {
        if (planNums[i] !== planNums[i - 1] + 1) {
          warnings.push(
            `Gap in plan numbering in ${dir}: plan ${planNums[i - 1]} → ${
              planNums[i]
            }`,
          );
        }
...
function findPhaseInternal(cwd, phase) {
  ...
  try {
    ...
    const hasContext = phaseFiles.some((f) =>
      f.endsWith("-CONTEXT.md") || f === "CONTEXT.md"
    );
    const hasVerification = phaseFiles.some((f) =>
      f.endsWith("-VERIFICATION.md") || f === "VERIFICATION.md"
    );
    ...
    return {
      ...
      phase_slug: phaseName
        ? phaseName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(
          /^-+|-+$/g,
          "",
        )
        : null,
...
function cmdInitPlanPhase(cwd, phase, includes, raw) {
  ...
  if (includes.has("uat") && phaseInfo?.directory) {
    ...
    try {
      const files = fs.readdirSync(phaseDirFull);
      const uatFile = files.find((f) =>
        f.endsWith("-UAT.md") || f === "UAT.md"
      );
      if (uatFile) {
...
function cmdInitResume(cwd, raw) {
  ...
  try {
    interruptedAgentId = fs.readFileSync(
...
function cmdInitMilestoneOp(cwd, raw) {
  ...
  try {
    ...
    for (const dir of dirs) {
      try {
        const phaseFiles = fs.readdirSync(path.join(phasesDir, dir));
        const hasSummary = phaseFiles.some((f) =>
          f.endsWith("-SUMMARY.md") || f === "SUMMARY.md"
        );
...
function cmdInitProgress(cwd, includes, raw) {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      if (
        !currentPhase && (status === "in_progress" || status === "researched")
      ) {
  ...
  switch (command) {
    ...
    case "milestone": {
      ...
      if (subcommand === "complete") {
        ...
        const milestoneName = nameIndex !== -1
    ...
    case "summary-extract": {
      ...
      const fields = fieldsIndex !== -1
        ? args[fieldsIndex + 1].split(",")
        : null;
