...
Template for `.planning/codebase/INTEGRATIONS.md` - captures external service
dependencies.

**Purpose:** Document what external systems this codebase communicates with.
Focused on "what lives outside our code that we depend on."
...
- [Service] - [What it's used for: e.g., "subscription billing, one-time
  payments"]
...
- [Service] - [Endpoint: e.g., "/api/webhooks/stripe"]
  - Verification: [e.g., "signature validation via
...
_Integration audit: [date]_ _Update when adding/removing external services_
```
...
_Integration audit: 2025-01-20_ _Update when adding/removing external services_
```
...
**Security note:** Document WHERE secrets live (env vars, Vercel dashboard,
1Password), never WHAT the secrets are.
</guidelines>
