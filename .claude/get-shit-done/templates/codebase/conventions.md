...
Template for `.planning/codebase/CONVENTIONS.md` - captures coding style and
patterns.

**Purpose:** Document how code is written in this codebase. Prescriptive guide
for Claude to match existing style.
...
_Convention analysis: [date]_ _Update when patterns change_
```
...
- Log error with context before throwing: logger.error({ err, userId }, 'Failed
  to process')
- Include cause in error message: new Error('Failed to X', { cause:
  originalError })
...
- Structured logging with context: logger.info({ userId, action }, 'User
  action')
...
_Convention analysis: 2025-01-20_ _Update when patterns change_
```
